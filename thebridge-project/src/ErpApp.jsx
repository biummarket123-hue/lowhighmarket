...
import { G, SF, S, INIT_DATA, DEFAULT_SETTINGS, DEFAULT_MANAGERS, DEFAULT_INV, STORAGE_KEYS } from "./constants.js";
...
function ErpApp() {
  ...
  const exportShipping = (selIds, courier) => {
    const COLS = {
      "CJ대한통운":{
        cols:["수취인명","수취인전화번호","수취인주소","품목명","박스수량","고객주문번호","보내는사람","보내는사람전화","보내는사람주소","배송메모"],
        row:(o,s,c)=>({
          "수취인명":o.customer,
          "수취인전화번호":o.phone||c.find(x=>x.name===o.customer)?.phone||"",
          "수취인주소":o.address||"",
          "품목명":o.items.map(i=>i.fabric+(i.color?" "+i.color:"")).join(" / "),
          "박스수량":1,
          "고객주문번호":o.id,
          "보내는사람":s.senderName,
          "보내는사람전화":s.senderPhone,
          "보내는사람주소":s.senderAddr,
          "배송메모":o.note||"",
        })
      },
      "경동화물":{
        cols:["받는분성명","받는분연락처","받는분주소","품명","수량(마)","단위","주문번호","보내는분","보내는분연락처","보내는분주소","메모"],
        row:(o,s,c)=>({
          "받는분성명":o.customer,
          "받는분연락처":o.phone||c.find(x=>x.name===o.customer)?.phone||"",
          "받는분주소":o.address||"",
          "품명":o.items.map(i=>i.fabric+(i.color?" "+i.color:"")).join(", "),
          "수량(마)":o.items.reduce((a,i)=>a+(i.qty||0),0),
          "단위":"마",
          "주문번호":o.id,
          "보내는분":s.senderName,
          "보내는분연락처":s.senderPhone,
          "보내는분주소":s.senderAddr,
          "메모":o.note||"",
        })
      },
